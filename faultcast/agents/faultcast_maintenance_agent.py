...
def classify_fault_severity(
    ...
    Returns:
        Dictionary with severity classification and priority
    """
    ...
    if not fault_type or fault_type.lower() in ["unknown", "none", ""]:
        ...
        # If fault type is unknown but confidence is high, treat as critical
        if confidence_score >= 0.7:
            ...
            priority = "scheduled"
            urgency_hours = 168
    # Classify based on fault type and confidence
    elif fault_type.lower() in ["normal", "no fault"]:
        severity = "normal"
        priority = "scheduled"
        urgency_hours = 168
...
def create_work_schedule(
    ...
    Returns:
        Dictionary with work schedule details and S3 location
    """
    if not S3_CLIENT:
        return {
